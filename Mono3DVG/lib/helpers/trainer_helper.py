...
class Trainer(object):
    def __init__(self,
        ...
        if cfg.get('resume_model', None):
            ...
            detr_model_path = r"F:\YY\mono3DVG\Mono3DVG\configs\checkpoint_best_MonoDETR.pth"
    ...
    def train_one_epoch(self, epoch):
        torch.set_grad_enabled(True)# enable grad
        self.model.train()
        # print(">>>>>>> Epoch:", str(epoch) + ":")

        progress_bar = tqdm.tqdm(total=len(self.train_loader), leave=(self.epoch+1 == self.cfg['max_epoch']), desc='iters')
        for batch_idx, (inputs, calibs, targets) in enumerate(self.train_loader):
            captions = targets["text"]["text"]
            ...
            # instanceID = targets['instance_id']  #  实例id  0
            # ann_id = targets['anno_id']  #  注释id  0

            for key in targets.keys():
                if key not in ['text']:
                    other_dict = targets[key]
                    for j in range(len(other_dict)):
                        single_dict = other_dict[j] 
                        for key in single_dict.keys():
                            single_dict[key] = single_dict[key].to(self.device)
            ...
            outputs = self.model(inputs, calibs, img_sizes, captions)

            mono3dvg_losses_dict = self.mono3dvg_loss(outputs, targets)  # 这是实际计算的Loss single

            weight_dict = self.mono3dvg_loss.weight_dict   #？？？？？这为什么是权重？ 预定义的？
            mono3dvg_losses_dict_weighted = [mono3dvg_losses_dict[k] * weight_dict[k] for k in mono3dvg_losses_dict.keys() if k in weight_dict] #loss权重*loss字典？
            mono3dvg_losses = sum(mono3dvg_losses_dict_weighted) #这个部分对损失项进行加权求和，得到最终的总损失 mono3dvg_losses。这是为了进行模型的反向传播和参数更新
    ...
    def prepare_targets(self, targets, batch_size):   
        ...
        for _ in range(30):
            ...
            big_targets_list.append(targets_list)
        return big_targets_list        
