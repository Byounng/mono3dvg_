...
class DepthPredictor(nn.Module):
    ...
    def forward(self, feature, mask, pos, text_memory, text_mask):   # 只用到了mask pos feature
        ...
        depth_logits = self.depth_classifier(src)   #  [1, 81, 24, 80])

        depth_probs = F.softmax(depth_logits, dim=1)
        weighted_depth = (depth_probs * self.depth_bin_values.reshape(1, -1, 1, 1)).sum(dim=1)#weighted_depth [1, 24, 80]

        # depth embeddings with depth positional encodings
        B, C, H, W = src.shape
        src = src.flatten(2).permute(2, 0, 1)   #[1920, 1, 256])
        mask = mask.flatten(1)
        ...
        return depth_logits, depth_embed, weighted_depth # 预测每个像素的深度信息 (depth_logits)。 深度嵌入 (depth_embed) 和加权深度 (weighted_depth)
