...
'''
    #jsfile 包括# ["exist_flag","img","json","truncated_state","occluded_state","alpha", "2dbox_xmin", "2dbox_xmin", "2dbox_xmin", "2dbox_xmin","3dbox_l","3dbox_w","3dbox_h","3dbox_x","3dbox_y","3dbox_z","rotation",
# "distanse","ordinal_number","place","direction","spatial_relation_nearest_type","spatial_relation_nearest_direction","spatial_relation_nearest_vehicle_direction","spatial_relation_nearest_vehicle_color"]
'''



def get_info_from_json(jsfile,p2_matix): 
    return Obj(jsfile,p2_matix)

class Obj(object):
    def __init__(self,jsfile,p2_matix) -> None:
        self.exist_flag = jsfile[0]
        self.img = jsfile[1]
        self.json = jsfile[2]
        self.truncated_state = jsfile[3]
        self.occluded_state = jsfile[4]
        self.alpha = jsfile[5]
        self.box2d = np.array((jsfile[6], jsfile[7], jsfile[8], jsfile[9]), dtype=np.float32)
        self.h = float(jsfile[10])
        self.w = float(jsfile[11])
        self.l = float(jsfile[12])
        self.pos = np.array((jsfile[13], jsfile[14], jsfile[15]), dtype=np.float32)
        self.ry = float(jsfile[16])
        self.dis_to_cam = float(jsfile[17])
        self.P2 = np.array(p2_matix).reshape(3,4)
        self.cu = self.P2[0, 2]
        self.cv = self.P2[1, 2]
        self.fu = self.P2[0, 0]
    def ry2alpha(self, ry, u):
        ...
        return alpha  
        # self.P2 = np.array(jsfile[18], dtype=np.float32)
    ...
    def rect_to_img(self, pts_rect):
        ...
        :return pts_img: (N, 2)
        """
        pts_rect_hom = self.cart_to_hom(pts_rect)  #3d xyz
